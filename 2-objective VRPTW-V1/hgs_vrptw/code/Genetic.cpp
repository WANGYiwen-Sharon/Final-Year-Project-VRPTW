...
//                std::cout << EP_total[0][0] << " " << EP_total[0][1] << std::endl;
	...
	{
		//std::cout << "number of nbIter is " << nbIter << std::endl;
//        if(nbIter % 10 == 0){
...
//                std::cout << "finishing choosing parents " << std::endl;
                ...
                Individual* newoffspring = bestOfSREXAndOXCrossovers(std::make_pair(parent1, parent2));
				params->nbMaxRoutes = newoffspring->myCostSol.nbRoutes - 1;
				lastnbRoutes = newoffspring->myCostSol.nbRoutes;
                // maxroutes = offspring->nbroutes

				while (decreasenbVehicles){
//                    std::cout << "params->nbMaxRoutes is " << params->nbMaxRoutes << std::endl;

					Individual* offspring = new Individual;
					*offspring = *newoffspring;  // 先复制一个
                    split->generalSplit(offspring, params->nbMaxRoutes); // split 新的
                    // localsearch-> //后续一样
	...
	//                std::cout << "finishing updating temperary population" << std::endl;
				...
				/* LOCAL SEARCH */
					// Run the Local Search on the new individual
				localSearch->run(newoffspring, params->penaltyCapacity, params->penaltyTimeWarp);
				/*if (nbIter == 16) {
					std::cout << "finishing local search" << std::endl;
				}*/
				//                std::cout << "finishing local search" << std::endl;
				...
				/* UPDATE MAX AND MIN COST*/
				if (newoffspring->myCostSol.penalizedCost < population->normalize_penalizedcost_min) population->normalize_penalizedcost_min = newoffspring->myCostSol.penalizedCost;
				if (newoffspring->myCostSol.nbRoutes < population->normalize_vehiclecost_min) population->normalize_vehiclecost_min = newoffspring->myCostSol.nbRoutes;
				if (newoffspring->myCostSol.penalizedCost > population->normalize_penalizedcost_max) population->normalize_penalizedcost_max = newoffspring->myCostSol.penalizedCost;
				if (newoffspring->myCostSol.nbRoutes > population->normalize_vehiclecost_max) population->normalize_vehiclecost_max = newoffspring->myCostSol.nbRoutes;

				/* UPDATE REFERENCE */
//                std::cout << "----- STARTING UPDATING REFERENCES" << std::endl;
				UpdateReference(newoffspring->myCostSol);

				//                std::cout << "finishing updating reference" << std::endl;
								/* INSERT INTO TEMPORARY POPULATION */
				//                std::cout << "offspring penalized cost is " << offspring->myCostSol.penalizedCost << std::endl;
				Individual* myoffspring = new Individual;
				*myoffspring = *newoffspring;
				//                std::cout << "myoffspring penalized cost is " << myoffspring->myCostSol.penalizedCost << std::endl;
				pop_tmp.push_back(myoffspring);
				//                std::cout << "finishing updating temperary population" << std::endl;
        ...
        //if (nbIter % 50 == 0)
		if(nbIter % 20 == 0) // not output intermediate information
        {
            
            std::cout << "nbIter: " << nbIter << std::endl;
			std::cout << "the EP are:" << std::endl;
			...
			/*std::cout << "the population are:" << std::endl;
			for (int i = 0; i < population->totalpop.size(); i++) {
				std::cout << "penalizedcost: " << population->totalpop[i]->myCostSol.penalizedCost << " vehiclecost: " << population->totalpop[i]->myCostSol.nbRoutes << std::endl;
			}*/
  //           /*if(nbIter == 150){
  ...
  //                  }
  //              }
		//		//std::cout << "----- FINISHING RESTART" << std::endl;
		//		/*for (int i = 0; i < population->totalpop.size(); i++) {
		//			std::cout << "penalizedcost: " << population->totalpop[i]->myCostSol.penalizedCost << " vehiclecost: " << population->totalpop[i]->myCostSol.nbRoutes << std::endl;
		//		}*/
  //          }
...
//		}
	...
	//	std::cout << "The " << i << "th solution in the non-dominated set" << std::endl;
	//	int sum = 0;
	//	for (int j = 0; j < EP[i]->myCostSol.nbRoutes; j++) {
	//		for (int p = 0; p < EP[i]->chromR[j].size(); p++) {
	//			std::cout << EP[i]->chromR[j][p] << " ";
	//			sum++;
	//		}
	//		std::cout << std::endl;
	//	}
