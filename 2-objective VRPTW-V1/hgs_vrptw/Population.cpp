...
//    std::cout << "Size of total population is " << feasibleSubpopulation.size() + infeasibleSubpopulation.size() << std::endl;
    for(int i = 0; i < feasibleSubpopulation.size() + infeasibleSubpopulation.size(); i++){
        if(i < feasibleSubpopulation.size()){
            double a = 1.0 * i / (feasibleSubpopulation.size() + infeasibleSubpopulation.size() - 1);
...
//        std::cout<< "i = " << i << std::endl;
        ...
        Minfastsort(v_dist, v_indx, (int)totalpop.size(), 20);
        totalpop[i]->neighbor = std::vector <int> (20);
