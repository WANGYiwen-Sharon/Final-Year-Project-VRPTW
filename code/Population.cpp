...
void Population::doLocalSearchAndAddIndividual(Individual* indiv)
{
	...
	// do multi-split to find minimum vehicle number for feasible solution
	if (indiv->isFeasible) {
		 //std::cout << "the vehicle number is " << indiv->myCostSol.nbRoutes << " and the distance is " << indiv->myCostSol.penalizedCost << std::endl;
		params->nbMaxRoutes = indiv->myCostSol.nbRoutes - 1;
		int lastnbRoutes = indiv->myCostSol.nbRoutes;
		bool decreasenbVehicles = true;

		Individual* insertindiv = new Individual;
		*insertindiv = *indiv;

		while (decreasenbVehicles) {
			//std::cout << "feasible" << std::endl;
			Individual* newindiv = new Individual;
			*newindiv = *indiv;  // 先复制一个
			split->generalSplit(newindiv, params->nbMaxRoutes); // split 新的

			/* LOCAL SEARCH */
			// Run the Local Search on the new individual
			localSearch->run(newindiv, params->penaltyCapacity, params->penaltyTimeWarp);

			if (newindiv->myCostSol.nbRoutes >= lastnbRoutes) {
				decreasenbVehicles = false;
			}
			else {
				//std::cout << "the vehicle number is " << newindiv->myCostSol.nbRoutes << " and the distance is " << newindiv->myCostSol.penalizedCost << std::endl;
				*insertindiv = *newindiv;
				lastnbRoutes = newindiv->myCostSol.nbRoutes;
				params->nbMaxRoutes = lastnbRoutes - 1;
			}
		}

		// update the min and max of distance and vehicle number

		if (insertindiv->myCostSol.penalizedCost < normalize_penalizedcost_min) normalize_penalizedcost_min = insertindiv->myCostSol.penalizedCost;
		if (insertindiv->myCostSol.nbRoutes < normalize_vehiclecost_min) normalize_vehiclecost_min = insertindiv->myCostSol.nbRoutes;
		if (insertindiv->myCostSol.penalizedCost > normalize_penalizedcost_max) normalize_penalizedcost_max = insertindiv->myCostSol.penalizedCost;
		if (insertindiv->myCostSol.nbRoutes > normalize_vehiclecost_max) normalize_vehiclecost_max = insertindiv->myCostSol.nbRoutes;
		// Add an individual
		//std::cout << "the vehicle number is " << insertindiv->myCostSol.nbRoutes << " and the distance is " << insertindiv->myCostSol.penalizedCost << std::endl;
		addIndividual(insertindiv, true);
		params->nbMaxRoutes = params->nbVehicles;
	}
	else if(params->rng() % 100 < (unsigned int)params->config.repairProbability) {
			...
			*insertindiv = *indiv;
			...
			while (decreasenbVehicles) {
				...
				*newindiv = *indiv;  // 先复制一个
				...
				// Run the Local Search on the new individual
				localSearch->run(newindiv, params->penaltyCapacity, params->penaltyTimeWarp);
				//std::cout << "infeasible to feasible " << newindiv->myCostSol.nbRoutes << std::endl;
				if (newindiv->myCostSol.nbRoutes >= lastnbRoutes) {
					decreasenbVehicles = false;
				}
				else {
			...
			// update the min and max of distance and vehicle number
			if (indiv->myCostSol.penalizedCost < normalize_penalizedcost_min) normalize_penalizedcost_min = indiv->myCostSol.penalizedCost;
			if (indiv->myCostSol.nbRoutes < normalize_vehiclecost_min) normalize_vehiclecost_min = indiv->myCostSol.nbRoutes;
			if (indiv->myCostSol.penalizedCost > normalize_penalizedcost_max) normalize_penalizedcost_max = indiv->myCostSol.penalizedCost;
			if (indiv->myCostSol.nbRoutes > normalize_vehiclecost_max) normalize_vehiclecost_max = indiv->myCostSol.nbRoutes;
			// Add an individual
...
{
//    std::cout << "Starting building population" << std::endl;
...
//    std::cout << "nofIndividuals is " << nofIndividuals << std::endl;
...
//    std::cout << "Starting building individuals" << std::endl;
	// Generate some individuals using the NEAREST construction heuristic 初始化用nearest建立solution
	for (int i = 0; i < nofNearestIndividualsToGenerate; i++)
	{
        ...
        // consider the individual as a subproblem and allocate weight vectors to it.
        
//        double a = 1.0 * i / (nofIndividuals - 1);
//        indiv.weight_vector   = std::vector<double>(2, 0);
//        indiv.weight_vector[0] = a;
//        indiv.weight_vector[1] = 1 - a;
        
        // insert the individuals to the population
		doLocalSearchAndAddIndividual(&indiv);
//        std::cout << "Finish building nearest construction" << std::endl;
...
//	printState(-1, -1);
	...
	for (int i = 0; i < nofFurthestIndividualsToGenerate; i++)
	{
        ...
        // consider the individual as a subproblem and allocate weight vectors to it.
//        double a = 1.0 * (i + nofFurthestIndividualsToGenerate) / (nofIndividuals - 1);
//        indiv.weight_vector   = std::vector<double>(2, 0);
//        indiv.weight_vector[0] = a;
//        indiv.weight_vector[1] = 1 - a;
        ...
        // insert the individuals to the population
		doLocalSearchAndAddIndividual(&indiv);
//        std::cout << "Finish building furthest construction" << std::endl;
	}
...
//	printState(-1, -1);
	...
	for (int i = 0; i < nofSweepIndividualsToGenerate; i++)
	{
        ...
        // consider the individual as a subproblem and allocate weight vectors to it.
//        double a = 1.0 * (i + nofFurthestIndividualsToGenerate + nofSweepIndividualsToGenerate) / (nofIndividuals - 1);
//        indiv.weight_vector   = std::vector<double>(2, 0);
//        indiv.weight_vector[0] = a;
//        indiv.weight_vector[1] = 1 - a;
        ...
        // insert the individuals to the population
		doLocalSearchAndAddIndividual(&indiv);
//        std::cout << "Finish building sweep construction" << std::endl;
	}
...
//	printState(-1, -1);
	...
	for (int i = 0; i < nofRandomIndividualsToGenerate; i++)
	{
		...
		split->generalSplit(&randomIndiv, params->nbVehicles);
        // consider the individual as a subproblem and allocate weight vectors to it.
//        double a = 1.0 * (i + nofFurthestIndividualsToGenerate + nofSweepIndividualsToGenerate + nofSweepIndividualsToGenerate) / (nofIndividuals - 1);
//        randomIndiv.weight_vector   = std::vector<double>(2, 0);
//        randomIndiv.weight_vector[0] = a;
//        randomIndiv.weight_vector[1] = 1 - a;
        
        // insert the individuals to the population
		doLocalSearchAndAddIndividual(&randomIndiv);
        //std::cout << "Finish building random construction" << std::endl;
	}
...
//	printState(-1, -1);
    ...
    // combine two subpopulations into one
   /* std::cout << "Size of feasible population is " << feasibleSubpopulation.size()<< std::endl;
	std::cout << "Size of infeasible population is " << infeasibleSubpopulation.size() << std::endl;*/
    for(int i = 0; i < 200; i++){
        if(i < feasibleSubpopulation.size()){
            double a = 1.0 * i / (200 - 1);
            feasibleSubpopulation[i]->weight_vector   = std::vector<double>(2, 0);
            feasibleSubpopulation[i]->weight_vector[0] = a;
            feasibleSubpopulation[i]->weight_vector[1] = 1 - a;
            totalpop.push_back(feasibleSubpopulation[i]);
            
            sum_weight_vector.push_back(feasibleSubpopulation[i]->weight_vector);
        }
        else{
            double a = 1.0 * i / (200 - 1);
            infeasibleSubpopulation[i - feasibleSubpopulation.size()]->weight_vector   = std::vector<double>(2, 0);
            infeasibleSubpopulation[i - feasibleSubpopulation.size()]->weight_vector[0] = a;
            infeasibleSubpopulation[i - feasibleSubpopulation.size()]->weight_vector[1] = 1 - a;
            totalpop.push_back(infeasibleSubpopulation[i - feasibleSubpopulation.size()]);
            sum_weight_vector.push_back(infeasibleSubpopulation[i - feasibleSubpopulation.size()]->weight_vector);
        }
    }
    
//    std::cout<< "The normalize_penalizeCost is " << normalize_penalizedcost_min << " " << normalize_penalizedcost_max << std::endl;
//    std::cout<< "The normalize_vehicleCost is " << normalize_vehiclecost_min << " " << normalize_vehiclecost_max << std::endl;
}
...
void Population::restart()
{
    ...
    totalpop.clear();
    //std::cout << "feasible population size: " << feasibleSubpopulation.size() << " infeasible population size: " << infeasibleSubpopulation.size() << " total population size: " << totalpop.size() << std::endl;
//    bestSolutionRestart = Individual(params);	// Generate a new initial population
	generatePopulation();
}
...
void Population::managePenalties()
{
	...
	for (int i = 0; i < static_cast<int>(totalpop.size()); i++)
	{
        if(totalpop[i]->isFeasible == false){
            totalpop[i]->myCostSol.penalizedCost = totalpop[i]->myCostSol.distance
                + params->penaltyCapacity * totalpop[i]->myCostSol.capacityExcess
                + params->penaltyTimeWarp * totalpop[i]->myCostSol.timeWarp;
        }
...
void Population::InitializeNeighborhood(){
    std::vector<double> v_dist   = std::vector<double>(totalpop.size(), 0);
    std::vector<int>    v_indx   = std::vector<int>(totalpop.size(), 0);
    
    for(int i = 0; i < totalpop.size(); i++){
//        std::cout<< "i = " << i << std::endl;
        for(int j = 0; j < totalpop.size(); j++){
            v_dist[j] = DistanceVectorNorm2(totalpop[i]->weight_vector, totalpop[j]->weight_vector);
            v_indx[j] = j;
        }
        
        // 20 is the number of neighbors
        Minfastsort(v_dist, v_indx, (int)totalpop.size(), 10);
        totalpop[i]->neighbor = std::vector <int> (10);

        for(int k = 0; k < 10; k++)
        {
            totalpop[i]->neighbor[k] = v_indx[k];
//            std::cout << "k is " << k << " and v_indx[k] is " << v_indx[k] << std::endl;
            
        }// save the indexes into neighborhood
    }
...
Individual * Population::SelectMatingPool(Individual subproblem)
{
//    Individual * individual1 ;
//    Individual * individual2 ;
    
//    updateBiasedFitnesses(totalpop);
    int place = params->rng() % subproblem.neighbor.size();
    Individual * indiv = totalpop[subproblem.neighbor[place]];
